...
folder_structure = Path('ses-baselineYear1Arm1')
# ###################################################
...
subs_fc = pd.read_csv('/data/project/impulsivity/pfactors/code/juseless/opts/grp2.txt')
# subs = subs_fc[(subs_fc['matched_group'] == 2)]
# subs = pd.DataFrame(subs)
subs = subs_fc.iloc[0:1000,0]
...
#     path2dataset = path2sprdataset / 'original' / data
    # "${1}/ses-baselineYear1Arm1/anat/${1}_ses-baselineYear1Arm1_space-fsLR32k_thickness.dscalar.nii"
    path2file = subject / folder_structure / f'{subject}{file}' 

    try:
        #print(f'Getting data: {path2file}')
        datalad.api.get(path2file.as_posix(), source='inm7-storage', recursive=True, ) #dont need this because we are in the dataset already, dataset = path2dataset.as_posix())
        #print('Got')
