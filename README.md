...
This is the code repo for our manuscript "Disentangling Brain-Psychopathology Associations: A Systematic Evaluation of Transdiagnostic Latent Factor Models" available at:
[text](https://www.biorxiv.org/content/10.64898/2025.12.21.695029v3.full.pdf+html)
...
This code is based on a prediction pipeline from https://github.com/MartinGell/Prediction_Reliability with minor changes and additions. For more detailed overivew see the repo.

<br />

Example files to run prediction script can be found in the directory. Example functional connectivity file in `/input/` and example behavioural data file in `/inpuy/pheno/`. Alongside with the two input files, two additional arguments are required: (i) the column to be predicted from the file with behavioural data and (ii) the prediction model to run (see `code/func/models.py` for available options). To recreate the findings of this, the user should choose the `ridgeCV_zscore_group_2Fold_confound_removal_wcategorical` model, which fits a Ridge regression on z-scored data after removing specified confounds (these can be specified in `prediction_nestedCV_val.py`). Therefore assuming the example files are actual ABCD data, the analyses for this project would recreated thus: 
```
